...
Since cestrel is based on FEniCS, most of the user input consists of definining
...
functional, the necessary modifications to optimize the problem in cestrel
...
suitable function space is chosen. This can, e.g., be achieved via
    ...
    config = cestrel.create_config('path_to_config')
...
Finally, we have to define the cost functional and the optimization problem
    ...
    opt_problem = cestrel.OptimalControlProblem(e, bcs, J, y, u, p, config)
    opt_problem.solve()

The only major difference between cestrel and fenics code is that one has to
...
For a detailed discussion of the features of cestrel and its usage we refer to the deomos.


Command line interface for mesh conversion
------------------------------------------

cestrel includes a command line interface for converting gmsh mesh files to
...
I develop cestrel as part of my PhD. I will try my best to document everything
...
cestrel is available on gitlab. As this project is part of my PhD, I am currently
...
cestrel is licensed under the ... license.
