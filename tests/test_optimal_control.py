...
config = cestrel.create_config('./config_ocp.ini')
...
ocp_cc = cestrel.OptimalControlProblem(F, bcs, J, y, u, p, config, control_constraints=cc)



def test_control_gradient():
	assert cestrel.verification.control_gradient_test(ocp) > 1.9
	assert cestrel.verification.control_gradient_test(ocp) > 1.9
	assert cestrel.verification.control_gradient_test(ocp) > 1.9
