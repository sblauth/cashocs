...
.. important::
    ...
    case letter is not considered for the .geo file created for remeshing.

    In our case of the .geo file, the characteristic length is defined as ``lc``,
    and this is used to specify the detail of the discretization via so-called
    size fields. Note, that this variable is also taken into consideration for
...
.. hint::
    ...
    file to .xdmf, the user may very well try to use this command.
...
In contrast to :ref:`demo_shape_poisson`, we cannot use a built-in mesh for this
