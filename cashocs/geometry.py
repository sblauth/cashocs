...
def regular_mesh(n=10, L_x=1.0, L_y=1.0, L_z=None):
	...
	.. math::
		\begin{alignedat}{2}
		&[0, \texttt{L_x}] \times [0, \texttt{L_y}] \quad &&\text{ in } 2D, \\
		&[0, \texttt{L_x}] \times [0, \texttt{L_y}] \times [0, \texttt{L_z}] \quad &&\text{ in } 3D.
		\end{alignedat}
	...
	The boundary markers are ordered as follows:
	  ...
	  - 2 corresponds to :math:`x=\texttt{L_x}`.

	  - 3 corresponds to :math:`y=0`.

	  - 4 corresponds to :math:`y=\texttt{L_y}`.

	  - 5 corresponds to :math:`z=0` (only in 3D).

	  - 6 corresponds to :math:`z=\texttt{L_z}` (only in 3D).
...
def regular_box_mesh(n=10, S_x=0.0, S_y=0.0, S_z=None, E_x=1.0, E_y=1.0, E_z=None):
	...
	.. math::
		\begin{alignedat}{2}
			&[\texttt{S_x}, \texttt{E_x}] \times [\texttt{S_y}, \texttt{E_y}] \quad &&\text{ in } 2D, \\
			&[\texttt{S_x}, \texttt{E_x}] \times [\texttt{S_y}, \texttt{E_y}] \times [\texttt{S_z}, \texttt{E_z}] \quad &&\text{ in } 3D.
		\end{alignedat}

	The boundary markers are ordered as follows:

	  - 1 corresponds to :math:`x=\texttt{S_x}`.

	  - 2 corresponds to :math:`x=\texttt{E_x}`.

	  - 3 corresponds to :math:`y=\texttt{S_y}`.

	  - 4 corresponds to :math:`y=\texttt{E_y}`.

	  - 5 corresponds to :math:`z=\texttt{S_z}` (only in 3D).

	  - 6 corresponds to :math:`z=\texttt{E_z}` (only in 3D).
