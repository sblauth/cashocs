...
&Omega; = (0,1)x(0,1), since this is built into descendal.

In the following, we will describe how to solve this problem
using descendal, using as much of the package as possible. Moreover,
...
We begin by importing fenics and descendal. For the sake of
...
interfere with descendal's output. Therefore, we use the line
...
works with usual fenics syntax. In descendal, we can quickly
...
To describe the state system in descendal, we use nearly standard
...
> for the use with descendal, the state variable y **must not**
...
for descendal is nearly identical to defining it with fenics,
...
directly solve it via descendal with the commands
